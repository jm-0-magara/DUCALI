...
export default function CategoryGrid({ artisans, viewMode, searchTerm, categoryName, clearFilters }: CategoryGridProps) {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
      {/* Results Summary */}
      <div className="mb-6">
        <p className="text-slate-300">
          Showing {artisans.length} {categoryName.toLowerCase()} artisan{artisans.length !== 1 ? 's' : ''}
      ...
      {artisans.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-2xl font-bold text-white mb-4">No Artisans Found</h3>
          <p className="text-slate-400 text-lg mb-6">
            No {categoryName.toLowerCase()} artisans found matching your criteria.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={clearFilters}
              className="px-6 py-2 bg-[#626F47] text-white rounded-lg hover:bg-[#A4B465] transition-colors"
            >
              Clear Filters
            </button>
            <Link
              href="/browse"
              className="px-6 py-2 border border-slate-600 text-slate-300 rounded-lg hover:border-[#A4B465] hover:text-[#A4B465] transition-colors inline-block text-center"
            >
              Browse All Artisans
            </Link>
          </div>
